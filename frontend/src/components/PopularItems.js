  ...
  return (
    <div className="card border-0 shadow-sm mb-4">
      <div className="card-body">
        <h5 className="card-title mb-3"><i className="bi bi-heart-fill text-danger me-2"/>Popular Items</h5>
        ...
        {!loading && (
          <div className="row g-3">
            {items.length === 0 && <div className="col-12 text-muted">No item data yet.</div>}
            {items.map(item => (
              <div key={item.name} className="col-6 col-md-3">
                <div className="card h-100 border-0 shadow-sm popular-item">
                  <div className="card-body d-flex flex-column justify-content-between p-3">
                    ...
                    <span className="badge bg-accent text-dark align-self-start px-3 py-2">{item.count} orders</span>
                  </div>
                </div>
              </div>
            ))}
